...
import { Calendar, Sparkles, AlertCircle, X } from 'lucide-react';
...
export const DateOfBirthInput: React.FC<DateOfBirthInputProps> = ({
  ...
  return (
    <div className="space-y-2.5" onPaste={handlePaste}>
      ...
      <div className="flex items-center space-x-2 rtl:space-x-reverse">
        ...
        <div className="w-20 sm:w-24">
          <div className="relative">
            ...
            />
            <span className="block text-[10px] text-center text-slate-500 font-semibold mt-0.5">
              {isAr ? 'اليوم' : 'Day'}
            </span>
          </div>
        </div>
        ...
        {(day || month || year) && (
          <button
            ...
            className="w-8 h-8 flex items-center justify-center text-slate-500 hover:text-rose-600 mb-4 rounded-lg hover:bg-slate-100 transition-colors"
      ...
      {errorMsg && (
        <div className="flex items-center space-x-1.5 rtl:space-x-reverse text-xs font-bold text-rose-600 animate-fade-in" role="alert">
      ...
      <div className="pt-1 flex items-center space-x-1 rtl:space-x-reverse overflow-x-auto scrollbar-none">
        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider flex-shrink-0 flex items-center space-x-0.5 rtl:space-x-reverse mr-1">
          <Sparkles className="w-3 h-3 text-amber-500 inline" aria-hidden="true" />
        ...
        {presets.map((p) => (
          <button
            ...
            className="px-2.5 py-1 min-h-[28px] rounded-lg text-[11px] font-bold bg-slate-100 hover:bg-bc-teal-50 text-slate-800 hover:text-bc-teal-900 border border-slate-200 hover:border-bc-teal-300 transition-all whitespace-nowrap"
            title={p.desc}
          >
            {p.label}
          </button>
